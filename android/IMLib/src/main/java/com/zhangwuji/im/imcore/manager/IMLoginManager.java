...
public class IMLoginManager extends IMManager {
     ...
     */
    public void relogin() {
        if(loginId>0 && !TextUtils.isEmpty(loginToken)){
    ...
    // 自动登陆流程
    public void login(String appId,LoginSp.SpLoginIdentity identity){
        if(identity == null){
            triggerEvent(LoginEvent.LOGIN_AUTH_FAILED);
            return;
        }
        this.appId=appId;
        loginId=identity.getLoginId();
        loginUserName = identity.getLoginName();
        loginToken = identity.getToken();
        identityChanged = false;

        int mLoginId = identity.getLoginId();
        // 初始化数据库
        DBInterface.instance().initDbHelp(ctx,mLoginId);
        User loginEntity = DBInterface.instance().getByLoginId(mLoginId);
        do{
            if(loginEntity == null){
                break;
            }
    ...
    public void login(String appId,int userid,String username, String token) {
        logger.i("login#login -> userName:%s", userid);
        this.appId=appId;
        loginId=userid;
    ...
    public void onLoginOk() {
        if (identityChanged) {
            LoginSp.instance().setLoginInfo(loginUserName,loginToken,loginId);
            identityChanged = false;
        }
