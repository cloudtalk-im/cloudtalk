...
namespace DB_PROXY {
    ...
    //            *pInfo = *it;
            ...
            //获取所有未读的系统通知，一次性全部推送给客户端。
            list<IM::Message::IMMsgData> lsMsg;
            CMessageModel::getInstance()->getNoReadSystemMessage(nUserId,1, lsMsg);
            for(auto it= lsMsg.begin(); it!=lsMsg.end(); ++it)
            {
                IM::Message::IMMsgData msg2;
                msg2.set_msg_data(it->msg_data());
                msg2.set_msg_type(it->msg_type());
                msg2.set_msg_id(it->msg_id());
                msg2.set_from_user_id(it->from_user_id());
                msg2.set_to_session_id(it->to_session_id());
                msg2.set_create_time(it->create_time());
                msg2.set_to_user_id(it->to_user_id());

                CImPdu* pPduMsg = new CImPdu;
                msg2.set_attach_data(msg.attach_data());

                pPduMsg->SetPBMsg(&msg2);
                pPduMsg->SetSeqNum(0);
                pPduMsg->SetServiceId(IM::BaseDefine::SID_MSG);
                pPduMsg->SetCommandId(IM::BaseDefine::CID_MSG_DATA);
                CProxyConn::AddResponsePdu(conn_uuid, pPduMsg);
                log("已完成系统通知发送 msgtype=%u,msgid=%u", it->msg_type(),it->msg_id());
            }
            CProxyConn::SendResponsePduList();
    ...
    void clearUnreadMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            //通知的已读标识工作要做好
            if(nSessionType==IM::BaseDefine::SessionType::SESSION_TYPE_NOTICE || nSessionType==IM::BaseDefine::SessionType::SESSION_TYPE_SYSTEM)
            {
                CMessageModel::getInstance()->setreadSystemMsg(nFromId,nUserId,msg.msg_id());
            }
