...
void CFileModel::getOfflineFile(uint32_t userId, list<IM::BaseDefine::OfflineFileInfo>& lsOffline)
{
    ...
    if (pDBConn)
    {
        string strSql = "select * from on_IMTransmitFile where toId="+int2string(userId) + " and status=0 order by created";
...
void CFileModel::addOfflineFile(uint32_t fromId, uint32_t toId, string& taskId, string& fileName, uint32_t fileSize)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into on_IMTransmitFile (`fromId`,`toId`,`fileName`,`size`,`taskId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?)";
...
void CFileModel::delOfflineFile(uint32_t fromId, uint32_t toId, string& taskId)
{
    ...
    if (pDBConn)
    {
        string strSql = "delete from on_IMTransmitFile where  fromId=" + int2string(fromId) + " and toId="+int2string(toId) + " and taskId='" + taskId + "'";
