        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (IM::BaseDefine::MsgType_IsValid(nMsgType))
            {
                if(nMsgLen != 0)
                {
                    ...
                    else if(nMsgType == IM::BaseDefine::MSG_TYPE_NOTICE_FRIEND || nMsgType == IM::BaseDefine::MSG_TYPE_NOTICE_SYSTEM ||  nMsgType == IM::BaseDefine::MSG_TYPE_SYSTEM)
                    {//对于系统通知的这块。只保存信息。并不处理session
                        uint32_t nRelateId = CRelationModel::getInstance()->getRelationId(nFromId, nToId, true);
                        nMsgId = pMsgModel->getMsgId(nRelateId);
                        if(nMsgId != INVALID_VALUE) {
                            pMsgModel->sendMessage(nRelateId, nFromId, nToId, nMsgType, nCreateTime, nMsgId, (string &) msg.msg_data());
                            log("sava system messages fromId=%u, toId=%u, type=%u, msgId=%u, sessionId=%u", nFromId, nToId, nMsgType, nMsgId, nSessionId);
                        }
                    }
