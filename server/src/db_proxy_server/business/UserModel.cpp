...
//获取好友资料更新的用户列表
void CUserModel::getChangedId(uint32_t& nLastTime,uint32_t& nReqId,list<uint32_t> &lsIds)
{
    ...
    if (pDBConn)
    {
        ...
        if(nLastTime == 0)
        {
            strSql = "select id, updated from on_IMUser where status != 3 and id in (select friUid from on_IMUserFriends where uid="+ int2string(nReqId)+" and status=1) ";
        }
        else
        {
            strSql = "select id, updated from on_IMUser where updated>=" + int2string(nLastTime)+" and id in (select friUid from on_IMUserFriends where uid="+ int2string(nReqId)+" and status=1) ";
        }
...
void CUserModel::getUsers(list<uint32_t> lsIds, list<IM::BaseDefine::UserInfo> &lsUsers)
{
    ...
    if (pDBConn)
    {
        ...
        string  strSql = "select * from on_IMUser where id in (" + strClause + ")";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                try {
                    ...
                    cUser.set_user_nick_name(pResultSet->GetString("nickname"));
                    cUser.set_user_domain(pResultSet->GetString("domain"));
                    cUser.set_user_real_name(pResultSet->GetString("realname"));
                ...
                {
                    log("error------- void CUserModel::getUsers:%s", exception1.what());
                }
            }
            delete pResultSet;
        }
...
bool CUserModel::getUser(uint32_t nUserId, DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        string strSql = "select * from  on_IMUser  where id="+int2string(nUserId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                try {
                    ...
                    cUser.strNick = pResultSet->GetString("nickname");
                    cUser.strDomain = pResultSet->GetString("domain");
                    cUser.strName = pResultSet->GetString("realname");
                ...
                {
                    log("error-------bool CUserModel::getUser:%s", exception1.what());
                }
            }
            delete pResultSet;
        }
...
bool CUserModel::updateUser(DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "update on_IMUser set `sex`=" + int2string(cUser.nSex)+ ", `nickname`='" + cUser.strNick +"', `domain`='"+ cUser.strDomain + "', `realname`='" + cUser.strName + "', `phone`='" + cUser.strTel + "', `email`='" + cUser.strEmail+ "', `avatar`='" + cUser.strAvatar + "', `sign_info`='" + cUser.sign_info +"', `departId`='" + int2string(cUser.nDeptId) + "', `status`=" + int2string(cUser.nStatus) + ", `updated`="+int2string(nNow) + " where id="+int2string(cUser.nId);
...
bool CUserModel::insertUser(DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into on_IMUser(`id`,`sex`,`nickname`,`domain`,`realname`,`phone`,`email`,`avatar`,`sign_info`,`departId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?,?,?,?,?)";
...
bool CUserModel::updateUserSignInfo(uint32_t user_id, const string& sign_info)
{
    ...
    if (db_conn) {
        ...
        string str_sql = "update on_IMUser set `sign_info`='" + sign_info + "', `updated`=" + int2string(now) + " where id="+int2string(user_id);
...
bool CUserModel::getUserSingInfo(uint32_t user_id, string* sign_info) {
    ...
    if (db_conn) {
        string str_sql = "select sign_info from on_IMUser where id="+int2string(user_id);
...
bool CUserModel::updatePushShield(uint32_t user_id, uint32_t shield_status) {
    ...
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update  on_IMUser  set `push_shield_status`="+ int2string(shield_status) + ", `updated`=" + int2string(now) + " where id="+int2string(user_id);
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
...
bool CUserModel::getPushShield(uint32_t user_id, uint32_t* shield_status) {
    ...
    CDBConn* db_conn = db_manager->GetDBConn("teamtalk_slave");
    if (db_conn) {
        string str_sql = "select push_shield_status from on_IMUser where id="+int2string(user_id);
        CResultSet* result_set = db_conn->ExecuteQuery(str_sql.c_str());
        if(result_set) {
            if (result_set->Next()) {
