                               ...
                               uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
	...
	if (nRelateId != INVALID_VALUE)
    {
        ...
        if (pDBConn)
        {
            string strTableName = "on_IMMessage_" + int2string(nRelateId % 8);
            string strSql;
            if (nMsgId == 0) {
...
void CMessageModel::getNoReadSystemMessage(uint32_t nUserId, uint32_t nPeerId ,list<IM::Message::IMMsgData>& lsMsg)
{
    uint32_t nRelateId = CRelationModel::getInstance()->getRelationId(nUserId,nPeerId,false);
    if (nRelateId != INVALID_VALUE)
    {
        ...
        if (pDBConn)
        {
            string strTableName = "on_IMMessage_" + int2string(nRelateId % 8);
            string strSql;
            strSql = "select * from " + strTableName + " force index (idx_relateId_status_created) where toId= " + int2string(nUserId) + " and type>100 and flag=0 order by created desc, id desc";
            ...
            if (pResultSet)
            {
                while (pResultSet->Next())
                {
                    IM::Message::IMMsgData cMsg;
                    cMsg.set_msg_id(pResultSet->GetInt("msgId"));
                    cMsg.set_from_user_id(pResultSet->GetInt("fromId"));
                    cMsg.set_create_time(pResultSet->GetInt("created"));
                    cMsg.set_to_user_id(pResultSet->GetInt("toId"));
                    cMsg.set_to_session_id(pResultSet->GetInt("toId"));

                    uint32_t msgtype=pResultSet->GetInt("type");
                 //   log("11111111111 msgType1=%u, msgType2=%u", msgtype, pResultSet->GetInt("type"));

                    IM::BaseDefine::MsgType nMsgType = IM::BaseDefine::MsgType(msgtype);
                    ...
                    if(IM::BaseDefine::MsgType_IsValid(nMsgType))
                    {
                        ...
                        cMsg.set_msg_data(pResultSet->GetString("content"));
                        lsMsg.push_back(cMsg);
                   //     log("system msg list. msgtype=%u,msgid=%u", nMsgType,pResultSet->GetInt("msgId") );
                    }
                    else
                    {
                        log("invalid msgType. userId=%u, peerId=%u, msgType=%u", nUserId, nPeerId, nMsgType);
                    }
                }
                delete pResultSet;
            }
 ...
 * 系统通知已读的标记
 */
bool CMessageModel::setreadSystemMsg(uint32_t nFromId, uint32_t nToId, uint32_t nMsgId)
{
    bool rv = false;
    ...
    if (nRelateId != INVALID_VALUE)
    {
        ...
        if (db_conn) {
            ...
            string str_sql = "update  "+strTableName+"  set `flag`= 1, `updated`=" + int2string(now) + " where msgId="+int2string(nMsgId);
            rv = db_conn->ExecuteUpdate(str_sql.c_str());
            if(!rv) {
                log("CMessageModel::setreadSystemMsg: update failed:%s", str_sql.c_str());
            }
            db_manager->RelDBConn(db_conn);
        } else {
            log("CMessageModel::setreadSystemMsg: no db connection for teamtalk_master");
        }
    }
}
 ...
 * GetShopId
 * Insert into on_IMMessage_ShopId%8
 */
...
uint32_t CMessageModel::getMsgId(uint32_t nRelateId)
{
    ...
    if(nMsgId==0)
    {
        ...
        if (pDBConn) {
            ...
            pDBManager->RelDBConn(pDBConn);
            string strKey = "msg_id_" + int2string(nRelateId);
            string strvalue=int2string(nMsgId);
            pCacheConn->set(strKey,strvalue);
        }
    }
    pCacheManager->RelCacheConn(pCacheConn);
    return nMsgId;
...
void CMessageModel::getLastMsg(uint32_t nFromId, uint32_t nToId, uint32_t& nMsgId, string& strMsgData, IM::BaseDefine::MsgType& nMsgType, uint32_t nStatus)
{
    ...
    if (nRelateId != INVALID_VALUE)
    {
        ...
        {
            string strTableName = "on_IMMessage_" + int2string(nRelateId % 8);
            string strSql = "select msgId,type,content from " + strTableName + " force index (idx_relateId_status_created) where relateId= " + int2string(nRelateId) + " and type<100 and status = 0 order by created desc, id desc limit 1";
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if (pResultSet)
            {
                while (pResultSet->Next())
